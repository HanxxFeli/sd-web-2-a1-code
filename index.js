...
let printCharNames = (array, idName) => { 
  array.forEach(character => { 
      idName.innerHTML += `<li>${character.name}</li>`
      console.log(character.name);
  })
};

printCharNames(users, funcList); // call the function for checking console.log of char names
...
let filterCharByAge = (array, age, charId) => { 
  array.forEach(character => { 
    if (character.age < age) { 
      if (character.name === "") { 
        charId.innerHTML += `<li>${character.name}</li>`
        console.log(character.name);
      } 
    }
  }) 
...
filterCharByAge(users, 40, ageFilterList); // call the function for checking age validation for char under 40

// 5. Add error handling to your functions that will log an error message using console.error() if any object doesn't have a "name" property. Display any error messages in the div with id "error-messages"
  
  // create a new user list with errors to test 
  const users2 = [
    { id: 1, name: "Luke Skywalker", age: 23 },
    { id: 2, name: "Darth Vader", age: 45 },
    { id: 3, name: "Princess Leia", age: 23 },
...
let errorHandlingFunc = (array, errorId, listId) => { 
  array.forEach(character => { 
    if (character.name === "") { 
      errorId.innerHTML += `<p>Character with age ${character.age} does not have a name</p>`
      console.error("Character does not have a name"); // log console error if char name doesnt exist
    } else 
      listId.innerHTML += `<li>${character.name}</li>`
      console.log(character.name);
  })
};

 errorHandlingFunc(users2, errorMessages, errorHandlingList); // call the function for error handling function

// 6. Test your error handling by creating a second array that's intentionally broken (missing name properties) and passing it to your functions. Verify that your error handling works correctly and displays errors in the div with id "broken-array-errors"

  // Create test array with missing values for name
const testUserArray = [
  {id: 11, name: "yvana", age: 27},
  {id: 12, name: "celine", age: 100},
  {id: 13, name: "", age: 28},
  {id: 13, name: "", age: 19},
]
...
const brokenArrayErrors = document.getElementById("broken-array-errors");
  ...
  //testing error handling of 2nd array
errorHandlingFunc(testUserArray, brokenArrayErrors, brokenArrayList);